...
class ConcretePointerBlock(Nonterm):
    def _validate(self):
        on_target_delete = None
        for cmd in self.val.commands:
            if isinstance(cmd, qlast.OnTargetDelete):
...
class ConcretePointerShort(Nonterm):
    ...
    # XXX: This can't work because it conflicts
    # with SetField... So probably the one with
    # PtrQuals shouldn't work either.
    # But maybe an alternate syntax??
    # def reduce_CreateComputablePointer(self, *kids):
    #     """%reduce
    ...
    # def reduce_CreateQualifiedComputablePointer(self, *kids):
    #     """%reduce
...
sdl_commands_block(
    ...
    ConcreteLinkBlock,
    ConcreteLinkShort,
    ConcretePointerBlock,
    ConcretePointerShort,
    ConcreteConstraintBlock,
    ConcreteConstraintShort,
    ConcreteIndexDeclarationBlock,
    ConcreteIndexDeclarationShort,
